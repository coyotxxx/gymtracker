...
class MigrationTest {
    ...
    @Test
    fun `migracja 56 do 60 wykonuje sie i waliduje schemat`() = runBlocking {
        buildV56Database()
        ...
        val tr = TraceReport("migration-56-to-60")
            .section("MIGRACJA")
            .verdict("ścieżka 56→57→58→59→60", "wykonana", "bez wyjątku")
            .verdict("walidacja schematu v60", "OK", "Room potwierdził zgodność")
    ...
    @Test
    fun `dane wstawione w v56 przezywaja migracje do v60`() = runBlocking {
        buildV56Database()
        ...
        val tr = TraceReport("migration-data-survival")
            ...
            .kv("ćwiczeń po migracji (v60)", exercises.size.toString())
        ...
        assertEquals("ćwiczenie przetrwało migrację 56→60", 1, exercises.size)
        assertEquals("nazwa ćwiczenia zachowana",
            "Wyciskanie testowe", exercises.first().name)
    }

    @Test
    fun `lancuch migracji 49 do 60 jest ciagly`() {
        ...
        assertEquals("łańcuch kończy się na wersji bazy danych", 60, version)
        assertTrue("11 migracji 49→60", migrations.size == 11)
    }
}
